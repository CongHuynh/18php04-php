1:
SELECT * FROM `products` INNER JOIN categories on products.productID = categories.categoryID 
WHERE categories.categoryName = "Guitars" AND products.listPrice > 500
2:
SELECT * FROM `products` WHERE `dateAdded` LIKE '2014-07-__%' AND `listPrice` > 300 
ORDER BY `listPrice` DESC
3:SELECT * FROM products INNER JOIN categories ON products.categoryID = categories.categoryID
...
ORDER BY discountPercent DESC LIMIT 4
6:
SELECT city FROM addressesINNER JOIN customers ON addresses.customerID = customers.customerID
